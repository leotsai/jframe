...
import org.jframe.web.demo.viewModel.EmployeeDetailViewModel;
import org.jframe.web.demo.viewModel.EmployeeEditViewModel;
import org.jframe.web.demo.viewModel.EmployeeIndexViewModel;
...
public class EmployeeController extends _DemoControllerBase {
...
//    private AdminSettingService adminSettingService;
    ...
    public ModelAndView index() {
        return super.tryView("demo-employee-index", () -> new EmployeeIndexViewModel().build(), mv -> {
    ...
    public ModelAndView list(PageRequest pageRequest, Long departmentId) {
        return super.tryView("demo-employee-list", () -> employeeService.searchByDepartmentId(pageRequest, departmentId));
    }

    @RequestMapping("/edit")
    @Authorize(permissions = {Codes.EDIT})
    public ModelAndView edit(Long employeeId, Long departmentId) {
        return super.tryView("demo-employee-edit", () -> new EmployeeEditViewModel().build(employeeId, departmentId));
    }

    @RequestMapping("/detail")
    @Authorize(permissions = {Codes.SELECT})
    public ModelAndView detail(Long employeeId) {
        return super.tryView("demo-employee-detail", () -> new EmployeeDetailViewModel().build(employeeId), mv -> {
            UserSession session = WebContext.getCurrent().getSession();
